...
// External customisation point for piranha::evaluate().
...
// Internal customisation point for piranha::evaluate().
template <typename T, typename U
#if !defined(PIRANHA_HAVE_CONCEPTS)
          ,
          typename = void
...
constexpr auto evaluate_impl(T &&x, const symbol_map<U> &sm, priority_tag<3>)
    PIRANHA_SS_FORWARD_FUNCTION((customisation::evaluate<T &&, U>)(::std::forward<T>(x), sm));

// Unqualified function call implementation.
template <typename T, typename U>
constexpr auto evaluate_impl(T &&x, const symbol_map<U> &sm, priority_tag<2>)
    PIRANHA_SS_FORWARD_FUNCTION(evaluate(::std::forward<T>(x), sm));
...
constexpr auto evaluate_impl(T &&x, const symbol_map<U> &sm, priority_tag<1>)
    PIRANHA_SS_FORWARD_FUNCTION((customisation::internal::evaluate<T &&, U>)(::std::forward<T>(x), sm));

// Lowest-priority: forward-construct the input value.
template <typename T, typename U>
constexpr auto evaluate_impl(T &&x, const symbol_map<U> &, priority_tag<0>)
    PIRANHA_SS_FORWARD_FUNCTION(remove_cvref_t<T>(::std::forward<T>(x)));
...
struct evaluate_functor {
    ...
    constexpr auto operator()(T &&x, const symbol_map<U> &sm) const
        PIRANHA_SS_FORWARD_MEMBER_FUNCTION(detail::evaluate_impl(::std::forward<T>(x), sm, detail::priority_tag<3>{}))
};
...
template <typename T, typename U>
using evaluate_t = decltype(::piranha::evaluate(::std::declval<T>(), ::std::declval<const symbol_map<U> &>()));
...
template <typename T, typename U>
PIRANHA_CONCEPT_DECL Evaluable = requires(T &&x, const symbol_map<U> &sm)
{
    ::piranha::evaluate(::std::forward<T>(x), sm);
};
