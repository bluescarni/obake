...
// if a mulhi() primitive for std::uint64_t is available.
namespace detail
{
...
struct OBAKE_DLL_PUBLIC kpack_data<::std::int32_t> {
    ...
    // The coded value limits, one for each size.
...
public:
    ...
    {
        if (obake_unlikely(size > ::std::size(detail::kpack_data<T>::deltas))) {
            ...
            obake_throw(::std::overflow_error,
                        ...
                        "was specified instead"_format(::obake::type_name<T>(),
                                                       ::std::size(detail::kpack_data<T>::deltas), size));
        }
    }
    ...
    kpacker &operator<<(const T &n)
    {
        ...
        // Get the limit.
        const auto lim = detail::kpack_data<T>::lims[m_size - 1u];

        // Check that n is within the limit.
        if constexpr (is_signed_v<T>) {
            if (obake_unlikely(n < -lim || n > lim)) {
                obake_throw(::std::overflow_error, "Cannot push the value {} to this Kronecker packer for the type "
                                                   "'{}': the value is outside the allowed range [{}, {}]"_format(
                                                       n, ::obake::type_name<T>(), -lim, lim));
            }
        } else {
            if (obake_unlikely(n > lim)) {
                obake_throw(::std::overflow_error, "Cannot push the value {} to this Kronecker packer for the type "
                                                   "'{}': the value is outside the allowed range [0, {}]"_format(
                                                       n, ::obake::type_name<T>(), lim));
            }
        }
        ...
        m_cur_prod *= detail::kpack_data<T>::deltas[m_size - 1u];
        ++m_index;
...
public:
    explicit kunpacker(const T &n, unsigned size) : m_value(n), m_size(size)
    {
        ...
        if (size == 0u) {
            ...
            if (obake_unlikely(size > ::std::size(detail::kpack_data<T>::deltas))) {
                obake_throw(::std::overflow_error,
                            ...
                            "maximum possible size is {}, but a size of {} was specified instead"_format(
                                ::obake::type_name<T>(), ::std::size(detail::kpack_data<T>::deltas), size));
            }

            // Get the limit.
            const auto lim = detail::kpack_data<T>::klims[m_size - 1u];
            ...
            if constexpr (is_signed_v<T>) {
                ...
                if (obake_unlikely(n > lim)) {
                    obake_throw(::std::overflow_error,
                                "The value {} passed to a Kronecker unpacker for the type "
                                "'{}' is outside the allowed range [0, {}]"_format(n, ::obake::type_name<T>(), lim));
                }
            }
        }
    }
    ...
    kunpacker &operator>>(T &out)
    {
        ...
        // Prepare cur_prod. This is the divisor in the remainder operation.
        const auto delta = detail::kpack_data<T>::deltas[m_size - 1u];
        m_cur_prod *= delta;
        ...
        // Compute the shifted counterpart of m_value, if necessary.
        const auto n = [this]() {
            if constexpr (is_signed_v<T>) {
                return m_value + detail::kpack_data<T>::klims[m_size - 1u];
            } else {
                return m_value;
            }
        }();

        // Helper to perform the division by a constant.
        ...
        if constexpr (is_signed_v<T>) {
            out = q_d - detail::kpack_data<T>::lims[m_size - 1u];
        } else {
            out = q_d;
        }
