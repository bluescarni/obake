...
struct key_is_zero_msvc {
    ...
    constexpr auto operator()(T &&x, const symbol_set &ss) const
        OBAKE_SS_FORWARD_MEMBER_FUNCTION(static_cast<bool>(detail::key_is_zero_impl(::std::forward<T>(x), ss,
...
template <typename T>
OBAKE_CONCEPT_DECL ZeroTestableKey = requires(T &&x, const symbol_set &ss)
{
