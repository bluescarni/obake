...
# Core deps.
sudo apt-get install build-essential cmake libgmp-dev libmpfr-dev wget clang curl libboost-dev
...
# GCC build.
cmake ../ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH=~/.local -DPIRANHA_BUILD_TESTS=YES -DCMAKE_CXX_FLAGS="-fconcepts --coverage" -DPIRANHA_WITH_STACK_TRACES=YES
make -j2 VERBOSE=1
# Run the tests.
ctest -V
